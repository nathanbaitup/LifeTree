...
export default function MoodJournal(props) {
    ...
    const heartRef = firestore().collection('heartRateList');
    ...
    // For loop that adds all heart rate data to their own arrays
    for (let i = 0; i < allHeartRates.length; i++) {
        allHRDates.push(allHeartRates[i].dateOfReading);
        allHRTimes.push(allHeartRates[i].timeOfReading);
        allHRDescriptions.push(allHeartRates[i].description);
        allHRQuantity.push(allHeartRates[i].heartRate);
    }
    ...
    allDates.forEach((day) => {
        ...
        // Checks if the day is the same as the date and then adds the heart rate data for the date to the calendar.
        // Adds a mark to the calendar.
        for (let i = 0; i < allHRDates.length; i++) {
            if (day === allDates[i]) {
                selectedHR = allHRQuantity[i];
                selectedDesc = allHRDescriptions[i];
                selectedTime = allHRTimes[i];
            }
        }
        allDatesObject[day] = {
            selected: true,
    ...
    const displayObsessionsAndHeartRate = (day) => {
        for (let i = 0; i < allObsessions.length; i++) {
            if (day.dateString === allDates[i]) {
                selectedObsession = allObsessions[i];
                for (let i = 0; i < allHRDates.length; i++) {
                    if (day.dateString === allHRDates[i]) {
                        selectedHR = allHRQuantity[i];
                        selectedDesc = allHRDescriptions[i];
                        selectedTime = allHRTimes[i];
                    }
    ...
    return (
        <ImageBackground source={require('../../../resources/img/background.png')} style={{ width: '100%', height: '100%', opacity: 50 }} >
            {/* Checks if a user has pressed on a calnedar entry to display the data. */}
            {entryPressed ? (
                <View style={journalStyles.popupMainContainer}>
                    <View style={[journalStyles.contentContainer, { padding: 25 }]}>
                        ...
                        <View style={journalStyles.popupContainer}>
                            <Text style={journalStyles.popupTitle}>Heart Rate: </Text>
                            <Text style={journalStyles.popupText}> {heartRate}</Text>
                        </View>
                        <View style={journalStyles.popupContainer}>
